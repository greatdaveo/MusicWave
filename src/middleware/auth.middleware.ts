...
export const protectedRoute = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(401).json({ status: 401, message: 'Not Authorized, please login' });
    }
};

export const userLoginStatus = async (req: Request, res: Response, next: NextFunction) => {
    const token = req.cookies.access_token;
    if (!token) {
    ...
    if (verifiedToken) {
        res.status(200).json({ status: 200, message: 'Retrieved a current logged in profile successfully' });
    } else {
        res.json(false);
    }
};
