...
export const registerUser = async (req: Request, res: Response, next: NextFunction) => {
    const { name, email, password, phoneNumber, accountType, country, countryCode, state, address } = req.body;

    if (!name || !email || !password || !accountType) {
        res.status(400).json({ status: 400, message: 'Please enter all required field' });
    }

    if (password.length < 6) {
        res.status(400).json({ status: 400, message: 'Password must be at least 6 characters' });
    }

    const existingUser = await UserModel.findOne({ email });
    if (existingUser) {
        res.status(400).json({ status: 400, message: 'This user exist, kindly login' });
    }
    ...
    if (newUser) {
        const { _id, name, email, password, phoneNumber, accountType, country, countryCode, state, address } = newUser;
...
export const loginUser = async (req: Request, res: Response, _next: NextFunction) => {
    ...
    if (!email || !password) {
        res.status(400).json({ status: 400, message: 'Please enter email and password' });
    }
    ...
    if (userExist) {
        ...
        if (userExist && correctPassword) {
            ...
            res.status(201).json({ status: 200, message: 'Login successfully', data: { token, user } });
        } else {
